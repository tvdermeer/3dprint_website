...
def test_update_product(client):
    ...
    response = client.put(
        ...
        json={"price": 15.00, "name": "Updated Product"},
    )
    assert response.status_code == 200
    data = response.json()
...
def test_check_stock(client):
    ...
    response = client.post(
        f"/api/v1/products/{product_id}/check-stock", json={"quantity": 5}
    )
    assert response.status_code == 200
    ...
    response = client.post(
        f"/api/v1/products/{product_id}/check-stock", json={"quantity": 15}
    )
    assert response.status_code == 200
